...
class TextToVideoPipeline(StableDiffusionPipeline):
    ...
    def DDPM_forward(self,  state: DDIMSchedulerState, x0, t0, tMax, generator, device, shape, text_embeddings):
        #status=to_test
        # rand_device = "cpu" if device.type == "mps" else device

        if x0 is None:
            return jax.random.normal(prpg, shape, dtype=text_embeddings.dtype)
            #return torch.randn(shape, generator=generator, device=rand_device, dtype=text_embeddings.dtype).to(device)
        else:
            eps = jax.random.normal(prpg, x0.shape, dtype=text_embeddings.dtype)
            ...
            alpha_vec = state.common.alphas_cumprod[tMax - 1]
    ...
    def prepare_latents(self, state: DDIMSchedulerState, batch_size, num_channels_latents, video_length, height, width, dtype, device, generator, latents=None):
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if t > skip_t:
                    ...
                    if not entered:
                        print(
                            f"Continue DDIM with i = {i}, t = {t}, latent = {latents.shape}, type = {latents.dtype}")
                        entered = True
                ...
                latent_model_input = self.scheduler.scale_model_input(
                    state, latent_model_input, timestep=t
                )
    ...
    def decode_latents(self, oarams, latents):
        ...
        video = jnp.clip((video / 2 + 0.5), 0, 1)
